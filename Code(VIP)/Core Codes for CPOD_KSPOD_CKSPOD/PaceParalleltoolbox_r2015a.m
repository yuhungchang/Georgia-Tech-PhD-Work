...
function [currCores, memRemain] = PaceParalleltoolbox_r2015a(varargin)
...
if maxCores && desiredCores < 2; %Don't open pools < 2 cores, but return memory information.
...
elseif strcmp(currEnv,'PACEcluster')
    ...
    interActivTest = any(~cellfun('isempty',strfind(jobCondsArr,'interactive')));
    if interActivTest
    ...
    if ~interActiv && ~vncServer %1 user probably won't open multiple interactive jobs on the
