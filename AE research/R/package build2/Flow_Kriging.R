...
design.range <- range(design.df[,1])
design.min <- min(design.df[,1])
design.df[,1] <- (design.df[,1] - design.min)/diff(design.range) # standardize
...
newCase <- (newCase - design.min)/diff(design.range) # standardize
