...
PredList =[61 62 63 64 65 66 67 68 69];  % prediction case (for validation)
...
%     ValiCase = zeros(size(DATA{1}));
    ...
    clc;
    if Para ==7
        DirOut=['../OutPut/Pressure/KSPOD_20180411_skip=' num2str(SKIP) '']; 
        ValiCase = ValiCase.*(1e-4);
        Prediction = Prediction.*(1e-4);
        Tempfile = fopen('PLTTemplate_Pressure.plt');
    elseif Para == 8
        DirOut=['../OutPut/Temperature/KSPOD_20180411_skip=' num2str(SKIP) ''];         
        Tempfile = fopen('PLTTemplate_Temperature.plt');
    elseif Para == 9
        DirOut=['../OutPut/Density/KSPOD_20180411_skip=' num2str(SKIP) ''];
        Tempfile = fopen('PLTTemplate_Density.plt');
    else         
        DirOut=['../OutPut/Others/KSPOD_20180411_skip=' num2str(SKIP) ''];
        Tempfile = fopen('PLTTemplate.plt');
    end
    eval(['OutFile=[''' 'KS_case' num2str(PredCase) '_mode=' num2str(CutNUM) '' '''];']);
