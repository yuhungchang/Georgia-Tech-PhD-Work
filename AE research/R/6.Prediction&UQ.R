...
#filepath <- "/home/proj/jeffwu/isye-ae/"
setwd(filepath)
...
for (i in 1:length(indvec)){
  ...
  #Compute cut-off
  #cs = cumsum(eigval)/sum(eigval) # edit on 07/17/2016
  cs = cumsum(eigval[eigval>0])/sum(eigval[eigval>0]) # add on 07/17/2016
  cs = pmax(cs-cut,0)
  cs[cs==0] = Inf
...
for (tt in 1:length(tvec)){
  ...
  independent.assumption <- FALSE
#   newCases <- matrix(des[1,],nrow=1)
  if (independent.assumption){
    predcoef <- GPpred(newCases, des, params$yMat, params$corr, params$mu, diag(diag(params$Tcov)), 2)  
  } else {
...
#   coefp-params$yMat[1,]
  ...
  for (i in 1:length(indvec)){
    ...
    sdsnap[[i]] <- 1.96 * sqrt( (PsiTlist[[i]][,1:numModes[i]]^2)%*%(varp[,(cModes[i]+1):cModes[i+1]]))
  }
