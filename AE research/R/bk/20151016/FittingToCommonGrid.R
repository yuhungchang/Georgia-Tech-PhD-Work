...
for(i in 1:30){
  CommonCoord.MinDistIndex[[i]] <- vector("list", 3)
}
...
for (i in 1:30){
  ...
  week = ceiling(i/6)
  if (i %in% wonkyInd){
    RealCoord <- read.table(paste0("RawData/", i,"/Week",week,"_",i,"_",2,".dat",sep=""), header = FALSE, skip = 1, sep = '\t', colClasses=rep("numeric",14), comment.char = "")[,1:2]
  }
  else{
...
#           return(which.min(SumSquare))
  ...
  save(CommonCoord.MinDistIndex, file = paste0("CommonCoord.MinDistIndex.",i, ".RData"))
}
