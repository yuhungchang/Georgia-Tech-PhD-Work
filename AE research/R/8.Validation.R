...
### Set work directory
setwd("/home/proj/jeffwu/isye-ae/Updated Case 31 CFD Validation Data/")

### Read data
tt <- 0
valid.dat <- read.table(paste0("Case31_CFD_",tt,".dat"), header = FALSE, 
                   skip = 14, sep = "", colClasses=rep("numeric",9), comment.char = "")
Coordinate.newcase <- valid.dat[,1:2]
...
save(CommonCoord.newcase, file = "RData/CommonCoord.newcase.RData")
...
FT = as.big.matrix(foreach(tm = available_file.tmp, .combine=cbind, .packages = c("colorRamps", "shape")) %dopar% {
  ...
  snapT <- read.table(paste0("Case31_CFD_",tm,".dat"), header = FALSE, 
                      skip = 14, sep = "", colClasses=rep("numeric",9), comment.char = "")
  snapT <- snapT[,ind-1]
